...
bool Cubes_GL_Uniform::init()
{
    // Shaders
    if (!create_shader(GL_VERTEX_SHADER, "cubes_gl_uniform_vs.glsl", &m_vs))
        return false;

    if (!create_shader(GL_FRAGMENT_SHADER, "cubes_gl_uniform_fs.glsl", &m_fs))
        return false;
...
bool Cubes_GL_Uniform::begin(void* window, GfxSwapChain* swap_chain, GfxFrameBuffer* frame_buffer)
{
...
void Cubes_GL_Uniform::end(GfxSwapChain* swap_chain)
{
