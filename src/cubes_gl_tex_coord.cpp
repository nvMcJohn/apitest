...
{
    // Shaders
    if (!CreateShader(GL_VERTEX_SHADER, "cubes_gl_tex_coord_vs.glsl", &m_vs))
        return false;

    if (!CreateShader(GL_FRAGMENT_SHADER, "cubes_gl_tex_coord_fs.glsl", &m_fs))
        return false;
...
bool Cubes_GL_TexCoord::Begin(GfxBaseApi* _activeAPI)
{
