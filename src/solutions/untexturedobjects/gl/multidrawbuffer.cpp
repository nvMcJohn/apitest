...
#include "multidrawbuffer.h"
#include "framework/gfx_gl.h"

// --------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
UntexturedObjectsGLMultiDrawBuffer::UntexturedObjectsGLMultiDrawBuffer(bool _useShaderDrawParameters)
: m_ib()
, m_vb()
, m_varray()
, m_drawid()
...
// --------------------------------------------------------------------------------------------------------------------
bool UntexturedObjectsGLMultiDrawBuffer::Init(const std::vector<UntexturedObjectsProblem::Vertex>& _vertices,
...
// --------------------------------------------------------------------------------------------------------------------
void UntexturedObjectsGLMultiDrawBuffer::Render(const std::vector<Matrix>& _transforms)
{
    ...
    glBindBuffer(GL_DRAW_INDIRECT_BUFFER, m_cmd_buffer);
    glBufferData(GL_DRAW_INDIRECT_BUFFER, count * sizeof(DrawElementsIndirectCommand), m_commands.data(), GL_DYNAMIC_DRAW);

    glMultiDrawElementsIndirect(GL_TRIANGLES, GL_UNSIGNED_SHORT, nullptr, count, 0);
}

// --------------------------------------------------------------------------------------------------------------------
void UntexturedObjectsGLMultiDrawBuffer::Shutdown()
{
    if (!mUseShaderDrawParameters) {
