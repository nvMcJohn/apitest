...
// --------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------
UntexturedObjectsGLMapUnsynchronized::UntexturedObjectsGLMapUnsynchronized()
: m_ib()
, m_vb()
, m_varray()
, m_drawid()
...
// --------------------------------------------------------------------------------------------------------------------
bool UntexturedObjectsGLMapUnsynchronized::Init(const std::vector<UntexturedObjectsProblem::Vertex>& _vertices,
...
// --------------------------------------------------------------------------------------------------------------------
void UntexturedObjectsGLMapUnsynchronized::Render(const std::vector<Matrix>& _transforms)
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        auto length = sizeof(Matrix);
        auto access = GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_UNSYNCHRONIZED_BIT;
        auto dst = glMapBufferRange(GL_SHADER_STORAGE_BUFFER, offset, length, access);
